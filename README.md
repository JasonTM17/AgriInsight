...
Phase 2's exact latest-50 feed and acknowledgement API plus same-origin BFF are verified in PR `#13` / CI `30425647823`. Phase 3 browser source is implemented locally with the lazy Field Ledger dialog, same-origin client, 30s open/visible poll, 90s stale clock, abort/cleanup, 401 session-expired, 403/404 blocked acknowledgement states, and stable retry state; focused Vitest, full web Vitest, typecheck, ESLint, production build, and the two-scenario Playwright list are green locally. Hosted browser/PostgreSQL/OIDC evidence and merge/release promotion remain pending.
...
- The alert-worker hardening reuses the released backend image. It has hosted acceptance and protected Docker Hub/GHCR publication for the worker slice, while the exact Phase 2 feed/ack API and Phase 3 browser alert panel are implemented in source and still await hosted browser acceptance and merge/release promotion. It makes no external deployment or semantic agriculture-alert claim.
