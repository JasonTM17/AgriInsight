...
1. Add authenticated internal query endpoint using the existing Spring-backed
   ...
   concealment rules.
2. Add BFF route with session auth, CSRF/origin/host validation, body and
   response limits, abort propagation, correlation IDs, and sanitized upstream
   errors.
3. Create the assistant panel with suggested agricultural questions,
   progressive request states, citation cards, data-as-of labels,
   refusal/error/retry states, keyboard support, focus management, and reduced
   motion.
...
- [x] Direct analytics access and token-bearing browser calls remain impossible.
- [x] Supplier and unauthorized FieldWorker journeys are denied; authorized
      personas receive only their farm/warehouse scope.
- [x] Desktop, tablet, and mobile CSS/a11y contracts, typecheck, lint, tests, and
      production build pass; real-browser release capture remains in Phase 4.
- [x] Logout, session expiry, cancellation, double-submit, slow provider, and
