...
1. Assistant no-query proof now listens to real Playwright request events during
   each capture. Any `POST /api/assistant/query` fails the capture, and the
   initial evidence-first heading must remain visible at both viewports.
2. The horizontal-overflow gate now rejects root or body scroll width beyond the
   ...
   containment cannot hide clipped interactive content from the detector.
