...
- `V22` alert storage is immutable. The isolated realtime alert worker is a
  private operational slice: V23-V27 target expected schema version 27, the
  worker startup gate independently pins successful V27 and the latest repeatable
  ...
  invalid-source-evidence index and does not replace the backfill. It has a
