...
- Use Flyway for schema changes and treat applied migrations as immutable history. `V22` is immutable; alert-worker hardening is V23-V27 with readiness expected at version 27. The worker startup gate independently pins successful V27 and the latest repeatable grant execution, while `AGRIINSIGHT_SCHEMA_EXPECTED_VERSION` stays backend readiness only. V23 must remain additive with `NOT VALID` source/evidence constraints and its required bounded backfill before worker enablement. V24-V26 remain one concurrent scan index per migration, while V27 is the readiness-only partial index for invalid source-evidence rows; all retain their named invalid-index recovery preconditions and V27 does not replace the V23 backfill.
