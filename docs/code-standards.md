...
- Keep the checked-in OpenAPI artifact deterministic. The live route inventory, shared `X-Correlation-Id`, and versioned `ETag` headers must remain aligned with the exported contract, including the exact realtime alert feed and acknowledgement operations.
...
- Use Flyway for schema changes and treat applied migrations as immutable history. `V22` is immutable; alert-worker hardening is V23-V28 and its worker startup invariant remains successful V28 plus the latest repeatable grant. Generic backend readiness advances through V29-V30 for open-only acknowledgement locking and the concurrent open-feed index. `AGRIINSIGHT_SCHEMA_EXPECTED_VERSION` is backend readiness only. V23 must remain additive with `NOT VALID` source/evidence constraints and its required bounded backfill before worker enablement. V24-V27 remain one concurrent scan index per migration, with V27 as the readiness-only partial index for invalid source-evidence rows; all retain their named invalid-index recovery preconditions. V28 must remain a forward acknowledgement-function repair that preserves its signature, security-definer search path, and grants while targeting the named unique constraint. Neither V27 nor V28 replaces the V23 backfill.
- Keep backend runtime data out of `artifacts/`.
- Keep the isolated realtime alert worker metadata-only. `realtime-alert-worker` runs under the non-web `realtime-worker` profile with the no-inheritance `agriinsight_alert_worker` login; Compose receives its password only through `AGRIINSIGHT_DB_ALERT_WORKER_PASSWORD`. Disable the legacy publisher/consumer path only in that service, retain the distinct DLT observer, and never grant it business tables, raw outbox payload, or error text. The verifier also requires the exact login topology, no inherited memberships, the narrow grants on `outbox_events`, `realtime_event_receipts`, `tenants`, `flyway_schema_history`, `realtime_operational_alerts`, and `realtime_operational_alert_scan_cursors`, plus the named FORCE-RLS policies on the alert and cursor tables; invalid source-evidence rows fail closed. The DLT path validates the bounded envelope, then in a dedicated transaction looks up `(tenant_id, event_id)` in `outbox_events`, uses the database `occurred_at`, and upserts only on a match; unmatched DLTs increment the unverified metric and log a stable event. Receipt recording and source attribution share a per-event advisory lock, so that path is database serialization, not exactly-once or broker ordering. Terminal observer forwarding must emit only a compact headerless marker, never a copied key/value/header or exception text. Its evaluator must keep REPEATABLE_READ, policy locks, durable cursors, bounded fair pages, current-condition recovery, hysteresis, saturation safety, and a configuration-bounded query timeout; the isolated profile's pgJDBC read timeout must outlive the 60-second query cap.
- Release workflows publish four first-party images (`agriinsight-python`, `agriinsight-backend`, `agriinsight-web`, `agriinsight-analytics-api`). PR CI builds the same four with `push: false`; protected release tags are semantic version plus `sha-<commit>` only, never `latest`.
...
- Treat the applied migration history as immutable: inventory tables, tenant
  ...
  and the completed/released follow-on isolated alert-worker hardening.
