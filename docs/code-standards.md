...
- Use Flyway for schema changes and treat applied migrations as immutable history. `V22` is immutable; alert-worker hardening is V23-V28 with readiness expected at version 28. The worker startup gate independently pins successful V28 and the latest repeatable grant execution, while `AGRIINSIGHT_SCHEMA_EXPECTED_VERSION` stays backend readiness only. V23 must remain additive with `NOT VALID` source/evidence constraints and its required bounded backfill before worker enablement. V24-V27 remain one concurrent scan index per migration, with V27 as the readiness-only partial index for invalid source-evidence rows; all retain their named invalid-index recovery preconditions. V28 must remain a forward acknowledgement-function repair that preserves its signature, security-definer search path, and grants while targeting the named unique constraint. Neither V27 nor V28 replaces the V23 backfill.
