...
public class ActivityDomainScopeResolver implements ScopeResolver.DomainScopeResolver {

    @Override
    public ScopeContext.Type type() {
        return ScopeContext.Type.ACTIVITY;
    }
    ...
    public ScopeResolver.DomainAccess access(
        ...
        if (roleGrants(roles, Role.FARM_MANAGER, permission)
                || roleGrants(roles, Role.FIELD_WORKER, permission)) {
