...
public class SupplierLifecycleController {
    ...
    private static final String DEACTIVATE = SUPPLIERS + "/{id}/deactivate";
    private static final String REACTIVATE = SUPPLIERS + "/{id}/reactivate";

    private final SupplierCommandService commands;
    private final ApiCommandFingerprintFactory fingerprints;

    public SupplierLifecycleController(
    ...
    ResponseEntity<SupplierResponse> deactivate(
            @PathVariable("id") UUID supplierId,
            @Valid @RequestBody SupplierLifecycleRequest body,
            @RequestHeader("Idempotency-Key") String idempotencyKey,
            @RequestHeader(HttpHeaders.IF_MATCH) String ifMatch,
            @AuthenticationPrincipal TenantPrincipal principal,
            HttpServletRequest request) {
        return changeActive(supplierId, body, idempotencyKey, ifMatch, principal, request, false);
    }

    @PostMapping("/{id}/reactivate")
    ResponseEntity<SupplierResponse> reactivate(
            @PathVariable("id") UUID supplierId,
            @Valid @RequestBody SupplierLifecycleRequest body,
            @RequestHeader("Idempotency-Key") String idempotencyKey,
            @RequestHeader(HttpHeaders.IF_MATCH) String ifMatch,
            @AuthenticationPrincipal TenantPrincipal principal,
            HttpServletRequest request) {
        return changeActive(supplierId, body, idempotencyKey, ifMatch, principal, request, true);
    }

    private ResponseEntity<SupplierResponse> changeActive(
            UUID supplierId,
            SupplierLifecycleRequest body,
        ...
        var executionRequest = fingerprints.create(
                ...
                correlationId);
        var command = new SupplierCommands.Lifecycle(
                expected.value(),
                new TenantAuditMetadata(
                        Optional.of(body.reasonCode()), Optional.of(correlationId)));
        var execution = active
                ? commands.reactivate(executionRequest, supplierId, command)
                : commands.deactivate(executionRequest, supplierId, command);
