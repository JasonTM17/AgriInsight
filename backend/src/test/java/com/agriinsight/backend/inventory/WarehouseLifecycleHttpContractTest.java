...
class WarehouseLifecycleHttpContractTest {

    private static final String BODY = "{\"reasonCode\":\"WAREHOUSE_LIFECYCLE_CHANGE\"}";
    ...
    void lifecycleRequiresIdempotencyAndStrongVersionHeaders() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/warehouses/{id}/deactivate", WAREHOUSE_ID)
        ...
        mockMvc.perform(post("/api/v1/warehouses/{id}/deactivate", WAREHOUSE_ID)
                        ...
                        .header(HttpHeaders.IF_MATCH, "\"3\"")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"reasonCode\":\"WAREHOUSE_LIFECYCLE_CHANGE\",\"cascade\":true}"))
                .andExpect(status().isBadRequest())
    ...
    void deactivateAndReactivateReturnCurrentStrongEtags() throws Exception {
        ...
        when(commands.deactivate(any(), any(), any()))
                .thenReturn(completed(200, warehouse(4, false)));
        when(commands.reactivate(any(), any(), any()))
                .thenReturn(completed(200, warehouse(5, true)));

        mutate("deactivate", "deactivate-warehouse-1", "\"3\"", "\"4\"");
        mutate("reactivate", "reactivate-warehouse-1", "\"4\"", "\"5\"");
    }
    ...
    void reusedLifecycleKeyWithChangedMeaningReturnsConflict() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/warehouses/{id}/deactivate", WAREHOUSE_ID)
                        .header(HttpHeaders.AUTHORIZATION, AUTHORIZATION)
                        .header("Idempotency-Key", "reused-warehouse-key")
                        .header(HttpHeaders.IF_MATCH, "\"3\"")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(BODY))
    ...
    private void mutate(
        ...
        mockMvc.perform(post("/api/v1/warehouses/{id}/" + action, WAREHOUSE_ID)
                        .header(HttpHeaders.AUTHORIZATION, AUTHORIZATION)
